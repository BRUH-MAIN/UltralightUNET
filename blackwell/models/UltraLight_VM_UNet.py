...
``mamba_ssm`` is deliberately NOT imported, even when it is available. A silent
per-machine backend switch would mean the code you debug is not the code that
produces the numbers. One backend everywhere. See models/mamba_pytorch.py.

BLACKWELL NOTE: the only difference from ../models/UltraLight_VM_UNet.py is the
trunc_normal_ import below. timm moved it to timm.layers in 0.9; the old path is
a deprecated shim. The cu128 environment installs a modern timm, so try the new
path first and fall back for old ones.
"""
...
except ImportError:  # timm < 0.9 (e.g. the 0.4.12 pinned in the cu117 env)
...
class PVMLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # PATCH (performance, not semantics): upstream invokes the *same* self.mamba
        # four times in sequence. Mamba treats the batch dimension as independent --
        # every op is either per-token or a scan along L -- so stacking the four
        # branches on the batch axis and doing one call computes exactly the same
        # thing. With a pure-PyTorch scan the loop issues many tiny CUDA kernels, so
        # this cuts the dominant cost (launch overhead) by 4x: ~14.7 h -> ~4 h for a
        # 250-epoch run, which is what brings it inside a Kaggle session.
        #
        # _forward_reference below keeps upstream's exact four-call form, and
        # tests/test_pvm_batching.py asserts the two agree.
        xs = torch.cat([x1, x2, x3, x4], dim=0)          # (4B, L, C/4)
        ys = self.mamba(xs)
        y1, y2, y3, y4 = torch.chunk(ys, 4, dim=0)

        x_mamba1 = y1 + self.skip_scale * x1
        x_mamba2 = y2 + self.skip_scale * x2
        x_mamba3 = y3 + self.skip_scale * x3
        x_mamba4 = y4 + self.skip_scale * x4
    ...
    def _forward_reference(self, x):
        """Upstream's forward, verbatim. Kept as the oracle for the batching patch."""
        if x.dtype == torch.float16:
            x = x.type(torch.float32)
        B, C = x.shape[:2]
        assert C == self.input_dim
