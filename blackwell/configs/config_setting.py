...
BLACKWELL v2 -- differences from ../configs/config_setting.py:
  * val_batch_size / test_batch_size are now settings (see the comment below).
    Speed only; training is unaffected.
  * data_path resolves relative to this file, so the notebook works regardless of
    the directory the kernel happens to start in.
