...
BLACKWELL v2 -- differences from ../train.py:
  ...
  * a preflight that fails loudly if the installed torch has no kernels for this
    GPU, which is exactly what happens on Blackwell with a cu117 build.
...
def main(config):
    ...
    # A cu117 build on a Blackwell card fails here rather than 20 minutes in with
    # an opaque "no kernel image is available for execution on the device".
    assert torch.cuda.is_available(), 'no CUDA device visible'
    cc = torch.cuda.get_device_capability(0)
    archs = torch.cuda.get_arch_list()
    log_info = (f'device: {torch.cuda.get_device_name(0)} (sm_{cc[0]}{cc[1]}), '
                f'torch {torch.__version__}, built for {archs}')
    print(log_info)
